...
if ((Get-Location).Path -eq "H:\") {
    ...
    Get-ChildItem "My Documents" | Copy-Item -Destination "$env:USERPROFILE\OneDrive - South Waikato District Council\Documents\" -Recurse
    Get-ChildItem | Where-Object {$_.Name -notin 'Desktop','Downloads','My Documents'} | Copy-Item -Destination "$env:USERPROFILE\OneDrive - South Waikato District Council\" -Recurse
    ...
    #Pictures
    Set-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders' -Name 'My Pictures' -Value "$env:USERPROFILE\OneDrive - South Waikato District Council\My Pictures"
    Get-ChildItem $env:USERPROFILE\Documents\Pictures | Move-Item -Destination "$env:USERPROFILE\OneDrive - South Waikato District Council\My Pictures\"
    ...
    #Music
    Set-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\User Shell Folders' -Name 'My Music' -Value "$env:USERPROFILE\OneDrive - South Waikato District Council\Documents\My Music"
